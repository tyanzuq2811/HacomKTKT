...
_FORMULA_STT = re.compile(r"^=", re.I)
...
def normalize_stt(value: str) -> str:
    ...
    if not s or s.startswith("="):
        return ""
    s = strip_accents(s).upper().replace(" ", "")
...
def canonical_id(sheet: str, code: str, name: str, ordinal: int = 0, path: str = "") -> str:
    sh = normalize_name(sheet)[:40]
    cd = normalize_code(code)
    nm = normalize_name(name)[:80]
    parent = normalize_name(path)[:60]
    base = cd or nm or f"ROW-{ordinal}"
    if parent:
        base = f"{parent}::{base}"
    return f"{sh}::{base}::{ordinal}" if sh else f"{base}::{ordinal}"
